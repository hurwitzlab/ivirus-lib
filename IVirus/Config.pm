...
has default_filename => (
    is      => 'ro',
    default => '/usr/local/ivirus/lib/conf/ivirus.yaml',
);
...
sub _build_filename {
    ...
    return $_[0] || $ENV{'IVIRUS_CONF'} || $self->default_filename;
}
...
IVirus::Config - Read local configuration information

=head1 SYNOPSIS

  use IVirus::Conf;

  my $config  = IVirus::Conf->new;
